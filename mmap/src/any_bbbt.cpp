...
void AnyBBBT::init() {
	...
	// initialize the ordering for the conditioned SUM subproblem
	std::vector<int> assignment;
	assignment.resize( m_problem->getN(), UNKNOWN );
	for (std::list<int>::iterator li = mapSearchOrder.begin();
			li != mapSearchOrder.end(); ++li) {
		assignment[*li] = 0;
	}

	Problem* cond = m_problem->conditioned(assignment);
	mex::vector<mex::Factor> fs(cond->getC());
	for (int i = 0; i < cond->getC(); ++i)
		fs[i] = cond->getFunctions()[i]->asFactor();

	mex::wmb _wmb(fs);
	_wmb.preprocess(); // minfill order
	if (_saved == false) {
		_order = _wmb.getOrder();
		_inducedWidth = _wmb.getInducedWidth();
		_saved = true;
	}

	std::cout << "Conditioned SUM subproblem" << std::endl
		<< "  - induced width: " << _inducedWidth << std::endl
		<< "  - variables:     " << cond->getN() << std::endl
		<< "  - factors:       " << cond->getC() << std::endl;

}

// select next unassigned MAP variable
int AnyBBBT::selectNextVar(const set<int>& M,
...
void AnyBBBT::bbDynamic(std::set<int> M, std::vector<int> assignment, std::set<int> F) {
	...
	if (M.empty()) { // new solution found (all MAP variables assigned)
		...
		double costSumExact = ELEM_NAN;
		double costSum = _lowerBound(assignment, costSumExact);
		...
		if (m_solutionCost < cost) {
			...
			std::cout << "["
				...
				<< std::setw(12) << m_expansions << " "
				<< std::setw(12) << m_solutionCost
				<< " (lb_sum=" << costSum << ", sum=" << costSumExact << ", map=" << costMap << ")"
				<< std::endl;
			std::cout.flush();
		}
...
// DONE: radu
double AnyBBBT::getMapAssignmentValue(const std::vector<int>& assignment) {
...
// recursive depth first branch and bound over the MAP variables
void AnyBBBT::bbStatic(std::list<int> M, std::vector<int> assignment) {
	...
	if (M.empty()) { // new solution found
		...
		if (m_solutionCost < cost) {
			...
			std::cout << "["
				...
				<< std::setw(12) << m_expansions << " "
				<< std::setw(12) << m_solutionCost
				<< " (lb_sum=" << costSum << ", sum=" << costSumExact << ", map=" << costMap << " | " << costSumExact*costMap << ")"
				<< std::endl;
			std::cout.flush();
		}
...
double AnyBBBT::_lowerBound(const std::vector<int>& assignment, double& exact) {

	// get the conditioned SUM problem
	Problem* cond = m_problem->conditioned(assignment);
	double lb = 0.0;
	...
	//int ib = (rand::probability() < 0.5) ? 4 : 1000;
	int ib = 4;
//	assert(_saved == true);
//	mex::wmb _wmb(fs);
//	_wmb.setOrder(_order);
//	_wmb.setIBound(ib);
//	_wmb.init();
//	lb = _wmb.lb();
	...
	//std::cout << "JG Lower bound for i=" << ib << " is " << lb << " (" << ELEM_DECODE(lb) << ")" << std::endl;
...
//	_ex.setIBound(1000);
//	_ex.init();
//	exact = ELEM_DECODE( _ex.lb() );

	// cleanup
	delete cond;

	return ELEM_DECODE(lb);
}
...
//
//// DONE: radu
//void AnyBBBT::_heuristicOR(AobbSearchNode* n) {
//
//	// Safety checks
...
//
//// DONE: radu
//bool AnyBBBT::_doProcess(AobbSearchNode* node) {
//	assert(node);
...
//
//// DONE: radu
//void AnyBBBT::_addCacheContext(AobbSearchNode* node, const set<int>& ctxt) const {
//
//	context_t sig;
...
//
//// DONE: radu
//bool AnyBBBT::_doCaching(AobbSearchNode* node) {
//
//	assert(node);
//	int var = node->getVar();
...
//
//// DONE: radu
//bool AnyBBBT::_doPruning(AobbSearchNode* node) {
//
//	assert(node);
//
...
//} // AOBB::doPruning
//
//bool AnyBBBT::_canBePruned(AobbSearchNode* n) {
//
...
//} // BBBT::canBePruned
//
//void AnyBBBT::getPathAssignment(AobbSearchNode* node, std::vector<int>& vars) {
//
...
//
//// DONE: radu
//AobbSearchNode* AnyBBBT::_nextLeaf() {
//
...
//
//// DONE: radu
//bool AnyBBBT::_generateChildrenAND(AobbSearchNode* n, vector<AobbSearchNode*>& chi) {
//
...
//
//// DONE: radu
//bool AnyBBBT::_generateChildrenOR(AobbSearchNode* n, vector<AobbSearchNode*>& chi) {
//
...
//
//// DONE: radu
//AobbSearchNode* AnyBBBT::_nextNode() {
...
//
//// DONE: radu
//bool AnyBBBT::_doExpand(AobbSearchNode* n) {
//	assert(n);
//	ao_expand.clear();
//
...
//
//// solve the problem
//double AnyBBBT::_ao(const std::vector<int>& assignment) {
//
//	// a temporary buffer
...
#ifdef UAI_COMPETITION
void AnyBBBT::writeSolution(std::vector<int>& sol) {
