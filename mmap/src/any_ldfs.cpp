...
//int AnyLRTAO::aostar(bool verbose) {
//
...
//		if ( !m_tips.empty() ) { 		// current partial tree has tip nodes
...
//			findBestPartialTree(); 	    // find the next partial tree
//		} else { 	// current partial tree is a solution tree
//
//			//std::cout << "Found " << m_leaves.size() << std::endl;
//
//			// propagate the values of its leaf nodes to the ancestors
//			for (std::vector<AobfSearchNode*>::iterator li = m_leaves.begin();
//					li != m_leaves.end(); ++li) {
//				AobfSearchNode* leaf = *li; // could also be OR sum nodes
//				//std::cout << "Leaf " << leaf->toString() << std::endl;
//				update(leaf);
//			}
//
//			// propagate the values of the tip nodes in case
//			for (std::vector<AobfSearchNode*>::iterator li = m_tips.begin();
//					li != m_tips.end(); ++li) {
//				AobfSearchNode* tip = *li;
//				update(tip);
//			}
//
...
//			}
//
//			findBestPartialTree(); // find the next best partial tree
//		}
//
...
// AO* search over the context minimal AND/OR graph restricted to MAP vars
int AnyLDFS::aostar(bool verbose) {
...
bool AnyLDFS::expand(AobfSearchNode *node) {
...
void AnyLDFS::expandAndRevise(AobfSearchNode* node) {
	// expand node n
...
void AnyLDFS::expandAndUpdate(AobfSearchNode* node) {
	// expand node n
	assert( node->isFringe() );
	assert( node->getChildren().empty() );
...
void AnyLDFS::update(AobfSearchNode* node) {
...
bool AnyLDFS::findBestPartialTree() {
...
bool AnyLDFS::isDeadend(AobfSearchNode* tip) {
...
bool AnyLDFS::revise(AobfSearchNode *node) {
...
bool AnyLDFS::setBestChild(AobfSearchNode* node) {
