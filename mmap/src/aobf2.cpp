...
void AOBF2::writeSolution() {
	std::vector<int> sol = m_assignment; // includes the dummy var
...
void AOBF2::init() {
	...
	if (m_options->heuristic == HEUR_WMB_JG) {
...
void AOBF2::getPathAssignment(AobfSearchNode* node, std::vector<int>& vars) {
...
bool AOBF2::isDeadend(AobfSearchNode* n, int var, int val) {
...
bool AOBF2::isDeadend(AobfSearchNode* node) {
	return false; // not used by AOBF2
}

// Revise the value of node 'n' based on its children values; log arithmetic
bool AOBF2::revise(AobfSearchNode *node) {
...
std::string AOBF2::context(int node_type, const std::set<int>& C) {
...
bool AOBF2::expand(AobfSearchNode *node) {
...
// DONE: radu
double AOBF2::heuristicOR(AobfSearchNode* n) {
...
void AOBF2::expandAndRevise(AobfSearchNode *node) {
...
bool AOBF2::findBestPartialTree() {

	// clear the existing tip nodes
	m_tips.clear();
...
AobfSearchNode* AOBF2::chooseTipNode() {
...
double AOBF2::_ao(int var, const std::vector<int>& assignment) {
	...
	while (n) {
		...
		n = _nextLeaf(); // throws SEARCH_TIMEOUT exception
	}
...
// DONE: radu
void AOBF2::_initAoSearch(int var) {
...
// DONE: radu
void AOBF2::_heuristicOR(AobbSearchNode* n) {
...
// DONE: radu
bool AOBF2::_doProcess(AobbSearchNode* node) {
	assert(node);
	if (node->getType() == NODE_AND) {
		int var = node->getVar();
		int val = node->getVal();
...
// DONE: radu
void AOBF2::_addCacheContext(AobbSearchNode* node, const set<int>& ctxt) const {
...
// DONE: radu
bool AOBF2::_doCaching(AobbSearchNode* node) {
...
// DONE: radu
bool AOBF2::_doPruning(AobbSearchNode* node) {
...
void AOBF2::_getPathAssignment(AobbSearchNode* node, std::vector<int>& vars) {
...
bool AOBF2::_canBePruned(AobbSearchNode* n) {
...
// DONE: radu
AobbSearchNode* AOBF2::_nextLeaf() {
...
// DONE: radu
bool AOBF2::_generateChildrenAND(AobbSearchNode* n, vector<AobbSearchNode*>& chi) {
...
// DONE: radu
bool AOBF2::_generateChildrenOR(AobbSearchNode* n, vector<AobbSearchNode*>& chi) {
...
// DONE: radu
AobbSearchNode* AOBF2::_nextNode() {
...
// DONE: radu
bool AOBF2::_doExpand(AobbSearchNode* n) {
	assert(n);
	ao_expand.clear();
