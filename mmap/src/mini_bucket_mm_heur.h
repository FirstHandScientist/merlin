...
#ifndef MINI_BUCKET_MPLP_HEUR_H_
#define MINI_BUCKET_MPLP_HEUR_H_
...
// Mini-bucket with cost shifting heuristic

class MiniBucketMMHeur : public Heuristic {
public:
	...
	double getGlobalUB() const {
		return (ELEM_DECODE(_mbe.ub())) * _p->getGlobalConstant();
	}
	...
	void setIbound(int ibound) {
		_mbe.setIBound(ibound);
	}
	// gets the i-bound
	int getIbound() const {
		return _mbe.getIBound();
	}
...
public:
	MiniBucketMMHeur(Problem* p, Pseudotree* pt, ProgramOptions* po, int ib);
	~MiniBucketMMHeur() {
	}
...
protected:
	...
	// preprocessing via MPLP
	void doMPLP();

};

inline bool MiniBucketMMHeur::isAccurate() {
	assert(m_pseudotree);
	return (m_pseudotree->getWidth() < (int) _mbe.getIBound());
}
